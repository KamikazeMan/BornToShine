...
void AMoonshinePlayerController::OpenRadialMenu()
{
	...
	RadialMenuOpenFrame = GFrameCounter;

	UE_LOG(LogTemp, Log, TEXT("Radial menu opened (%d segments) frame=%llu"), Infos.Num(), RadialMenuOpenFrame);
}
...
void AMoonshinePlayerController::CloseRadialMenu()
{
	...
	// Ignore phantom release generated by SetInputMode in the same frame we opened
	if (GFrameCounter <= RadialMenuOpenFrame)
	{
		UE_LOG(LogTemp, Warning, TEXT("Controller: CloseRadialMenu IGNORED phantom release (open frame=%llu, current=%llu)"),
			RadialMenuOpenFrame, GFrameCounter);
		return;
	}
