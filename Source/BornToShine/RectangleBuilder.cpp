...
void URectangleBuilderComponent::CalculateRidgePostLayout()
{
    ...
    // Building width: measure from top plates via global PhaseManager query,
    // filtered by proximity to THIS building's rim boards.
    ...
    // The proximity filter prevents adjacent buildings (in-line) from polluting
    ...
    if (DTPCount >= 2 && AConstructionPhaseManager::Instance)
    {
        ...
        float EndA_RidgeProj = FVector::DotProduct(EndBoardA->GetActorLocation(), RidgeFwd);
        float EndB_RidgeProj = FVector::DotProduct(EndBoardB->GetActorLocation(), RidgeFwd);
