...
void UBuildingComponent::BeginPlay()
{
	...
	if (!HasPieceType(EPieceType::RoofSheathing))
	{
		AvailablePieceTypes.Add(ARoofSheathing::StaticClass());
		UE_LOG(LogTemp, Warning, TEXT("BuildingComponent: Auto-added RoofSheathing to AvailablePieceTypes"));
	}
	...
	for (const FPieceTypeInfo& EditorInfo : PieceTypeInfos)
	{
		...
		case EPieceType::RoofSheathing:  AutoClass = ARoofSheathing::StaticClass(); break;
		default: break;
		}
	...
	for (int32 i = 0; i < AvailablePieceTypes.Num(); i++)
	{
		...
		if (AvailablePieceTypes.IsValidIndex(i) && AvailablePieceTypes[i])
		{
			...
			if (CDO)
			{
				...
				case EPieceType::RoofSheathing:  Info.Subtitle = TEXT("4x8"); break;
				default: break;
				}
			}
